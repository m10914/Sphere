...
HRESULT CManipulator::Create(LPDIRECT3DDEVICE9 pd3dDevice, wchar_t* file)
{
	...
	for( i = 0; i < numOfChains; i++ )
	{
		...
		cube[i]->Create(i, pd3dDevice, length, initangle, width, restrAngX, restrAngY, lvOffset, displace, direction, coefficient,mass,bTilt,bPressure,bLaser);
		...
		for(p=startY; p >= cube[i]->restrictAngleY.x; p -= iter)
		{
			//-------------------------------------------------------------------------
			{
				...
				if(i==fini)
				{
					...
					D3DXVECTOR3 vDiffer = vOut - vDiff;
					float distance = D3DXVec3Length(&vDiffer);
					if(distance < fMinDist)
					{
						fMinDist = distance;
						for(j=0;j<numOfChains;j++) fMinRes[j] = fRes[j];
					}
					if(distance < eps)
					{
						return true;
...
HRESULT CManipulator::InversedKinematicsPrecalc(LPWSTR limbName, LPWSTR filename)
{
	int i,j;
	HRESULT hr;
	...
	float length = GetLimbLength(limbName);

	arrPrecalc = new vector<D3DXVECTOR2>[123];

	return S_OK;
}

HRESULT CManipulator::InversedKinematics(D3DXVECTOR3 vecDesc, LPWSTR limbName, BOOL bTryToReach)
{
	int i,j;
	HRESULT hr;
	...
	//reach functional
	fMinDist = 999999;
	fMinRes = new D3DXVECTOR2[numOfChains];
	for(i=0;i<numOfChains;i++) fMinRes[i] = D3DXVECTOR2(0,0);

	
	//---------------------------------------------------------------------------

	int index1 = GetChainIndexByID(limb->firstchain);
	int index2 = GetChainIndexByID(limb->lastchain);
	if( findpoint_recursive(index1,index2,vecDesc,fRes,matEmpty) )
	{
