...
public:
	...
	HRESULT Render(LPDIRECT3DDEVICE9 pd3dDevice, D3DXMATRIX* mat);
	...
	D3DXMATRIX* GetChainMatrix(int iID, cprimitive* pCube, D3DXMATRIX mTransform, float quant = 1.f);
	D3DXMATRIX* GetChainMatrix(int iID, float quant = 1.f);
	D3DXVECTOR3* GetLimbCoord(LPWSTR limbname);
	D3DXVECTOR3* GetChainCoord(int iID);
...
protected:
	...
	HRESULT DrawChain(cprimitive* pCube, LPDIRECT3DDEVICE9 pd3dDevice, D3DXMATRIX* mat);
