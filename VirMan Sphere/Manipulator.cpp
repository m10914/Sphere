...
HRESULT CManipulator::FrameMove(float fElapsedTime, float fTime)
{
	...
	for(i=0;i < numOfChains; i++)
	{
		cprimitive* prim = cube[i];
		myvec = GetChainCoord(i);
		prim->oldCoord = prim->curCoord;
		prim->curCoord = *myvec;
		delete myvec;

		prim->vecMove = prim->curCoord - prim->oldCoord;
	}
...
// Desc:
//---------------------------------------------------------------------------------------------
HRESULT CManipulator::Render(LPDIRECT3DDEVICE9 pd3dDevice, D3DXMATRIX* mat)
{
	int i;
	...
	for(i=0; i<cube.size(); i++)
	{
		DrawChain(cube[0], pd3dDevice, mat);
	}
...
D3DXVECTOR3* CManipulator::GetChainCoord(int iID)
{
	D3DXMATRIX matRotRes;
	D3DXMatrixIdentity(&matRotRes);

	D3DXVECTOR3* retMat = GetLimbCoord(iID, cube[0], matRotRes);
...
D3DXMATRIX* CManipulator::GetChainMatrix(int iID, float quant)
{
	D3DXMATRIX matRotRes;
	D3DXMatrixIdentity(&matRotRes);

	D3DXMATRIX* retMat = GetChainMatrix(iID, cube[0], matRotRes, quant);

	return retMat;
}

D3DXMATRIX* CManipulator::GetChainMatrix(int iID, cprimitive* pCube, D3DXMATRIX mTransform, float quant)
{
	D3DXMATRIX* retMat;
	...
	//cooking current matrix
	D3DXMatrixTranslation(&offsetTrans,pCube->vOffset.x*quant, pCube->vOffset.y*quant, pCube->vOffset.z*quant);
	worldMatrix = pCube->GetRotationXMatrix() * pCube->GetRotationYMatrix() * offsetTrans;
	worldMatrix = worldMatrix * mTransform;

	//cooking ancestor matrices
	D3DXMatrixTranslation(&matTrans, 0, pCube->fLength*quant, 0);
	mTransformAncestors =  matTrans * pCube->GetRotationXMatrix() * pCube->GetRotationYMatrix() * offsetTrans * mTransform;
	...
	if(pCube->pSiblings != NULL)
	{
		retMat = GetChainMatrix(iID, pCube->pSiblings, mTransform, quant);
		if(retMat != NULL) return retMat;
	}

	if(pCube->pFirstChild != NULL)
	{
		retMat = GetChainMatrix(iID, pCube->pFirstChild, mTransformAncestors, quant);
		if(retMat != NULL) return retMat;
	}
...
// Desc:
//---------------------------------------------------------------------------------------------
HRESULT CManipulator::DrawChain(cprimitive* pCube, LPDIRECT3DDEVICE9 pd3dDevice, D3DXMATRIX* mat)
{
	D3DXMATRIX matRotRes;
	D3DXMatrixIdentity(&matRotRes);
	...
	{
		D3DXMATRIX* tempMat =GetChainMatrix(i);
		D3DXMATRIX resMat = *tempMat * *mat;
		pd3dDevice->SetTransform(D3DTS_WORLD, &resMat);
		if(i == selectedChain)
			cube[i]->Render(pd3dDevice, true);
		else
			cube[i]->Render(pd3dDevice);
